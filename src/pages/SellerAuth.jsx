...
const SellerAuth = () => {
    ...
    try {
      if (isLogin) {
        ...
        if (response.data.success) {
          ...
          setIsLogin(true);
          setFormData({ email: formData.email, password: '', confirmPassword: '', firstName: '', lastName: '', phone: '', country: '', businessName: '' });
        }
      }
    } catch (error) {
  ...
  const toggleMode = () => {
    ...
    setError('');
    setSuccess('');
    setFormData({ email: '', password: '', confirmPassword: '', firstName: '', lastName: '', phone: '', country: '', businessName: '' });
  };
    ...
    <div className="min-h-screen bg-[#F6F9FC] flex items-center justify-center p-4 relative">
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs font-semibold text-neutral-400 font-inter tracking-widest uppercase">Seller Portal</p>
        </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl shadow-lg p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className={inputClass} placeholder="Jane" />
                  </div>
                  <div>
                    <label className={labelClass}>Last Name *</label>
                    <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className={inputClass} placeholder="Smith" />
                  </div>
                </div>
                <div>
                  <label className={labelClass}>Business Name *</label>
                  <input type="text" name="businessName" value={formData.businessName} onChange={handleInputChange} className={inputClass} placeholder="Jane's Design Studio" />
                </div>
                <div>
            ...
            <div>
              ...
              <input type="email" name="email" value={formData.email} onChange={handleInputChange} className={inputClass} placeholder="jane@example.com" />
            </div>
