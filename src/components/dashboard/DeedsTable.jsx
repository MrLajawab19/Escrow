...
function getBuyerFacingStatus(deed) {
...
function filterDeeds(deeds, filterKey) {
  switch (filterKey) {
    case 'in_progress':
      return deeds.filter(d => ['IN_PROGRESS', 'ESCROW_LOCKED', 'ACTIVE'].includes(d.status));
    case 'pending_review':
      return deeds.filter(d => d.status === 'SUBMITTED');
    case 'completed':
      return deeds.filter(d => ['CONFIRMED', 'CLOSED'].includes(d.status));
    case 'disputed':
      return deeds.filter(d => ['DISPUTED', 'ARBITRATING', 'ARBITRATED', 'ESCALATED'].includes(d.status));
    case 'cancelled':
      return deeds.filter(d => ['CANCELLED'].includes(d.status));
    default:
      return deeds;
  }
}

const DeedsTable = ({ deeds, onViewAllDeeds }) => {
  ...
  const handleDeedClick = (deed) => {
    ...
    navigate(`/buyer/deed/${deedId}`);
  };

  const getSellerName = (deed) => {
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-2xl shadow-sm overflow-hidden mt-6">
      ...
      <div className="px-6 py-4 flex items-center justify-between border-b border-neutral-100">
        <h2 className="text-base font-bold text-navy-900">My Deeds</h2>
        <button
      ...
      <div className="divide-y divide-neutral-100">
        {filteredDeeds.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-sm font-semibold text-navy-900 mb-1">No deeds found</p>
            <p className="text-xs text-neutral-400">
              {activeFilter === 'all' ? 'Create your first deed to get started.' : `No ${filterTabs.find(t => t.key === activeFilter)?.label?.toLowerCase()} deeds.`}
            </p>
          </div>
        ) : (
          filteredDeeds.slice(0, 5).map((deed, index) => {
            const icon = getServiceIcon(index);
            const displayStatus = getBuyerFacingStatus(deed);
            return (
              <div
                key={deed.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-navy-900 truncate">{deed.title || 'Untitled Deed'}</p>
                  <p className="text-xs text-neutral-400 mt-0.5">Deed ID: {getDeedId(deed)}</p>
                </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="w-5 h-5 rounded-full bg-neutral-200 flex items-center justify-center text-[10px] font-bold text-neutral-600">
                        {deed.seller?.profileImage ? (
                          <img
                            src={`${import.meta.env.VITE_API_URL}${deed.seller.profileImage}`}
                            alt=""
                            className="w-full h-full rounded-full object-cover"
                          />
                        ) : (
                          getSellerName(deed).charAt(0)
                        )}
                      </div>
                      <p className="text-xs font-medium text-neutral-700 truncate max-w-[90px]">{getSellerName(deed)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="hidden sm:block text-right min-w-[80px]">
                  ...
                  <p className="text-sm font-semibold text-navy-900">
                    {formatCurrency(deed.amount || 0, deed.currency || 'INR')}
                  </p>
                </div>
