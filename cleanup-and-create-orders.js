...
async function cleanupAndCreateOrders() {
  try {
    ...
    const deleteResult = await Order.destroy({ where: {} });
    console.log(`🗑️  Deleted ${deleteResult} existing orders.`);
    
    // Create exactly 4 mock orders in different statuses to test all action buttons
    const mockOrders = [
      {
        id: uuidv4(),
        buyerId: buyer.id,
        ...
        status: 'ESCROW_FUNDED', // Buyer can: Cancel Order, Seller can: Accept/Reject
        escrowAmount: 150.00,
    ...
    // Verify final count
    const finalCount = await Order.count();
    console.log('');
    console.log(`📊 Database now contains exactly ${finalCount} orders.`);
    console.log('');
    console.log('🎯 Perfect! Now you have exactly 4 orders to test all action buttons:');
    console.log('');
    console.log('1. 📋 ESCROW_FUNDED Order ($150):');
    console.log('   - Buyer can: Cancel Order');
    console.log('   - Seller can: Accept/Reject Order');
    console.log('');
    console.log('2. 📝 PLACED Order ($500):');
    console.log('   - Buyer can: Cancel Order');
    console.log('   - Seller can: Accept/Reject Order');
    console.log('');
    console.log('3. ✅ SUBMITTED Order ($300):');
    ...
    console.log('');
    console.log('4. 🔄 IN_PROGRESS Order ($800):');
    console.log('   - Seller can: Submit Delivery');
    console.log('   - Buyer can: Cancel Order (if allowed)');
    console.log('');
    console.log('🚀 Both buyer and seller dashboards will now show exactly these 4 orders!');
